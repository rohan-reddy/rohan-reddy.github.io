...
image: images/fig-gpu-optimization-output-1.png 
...
### Assumed background
I will assume the reader understands the basics of the CUDA programming model. If not, I recommend reading the first 6 chapters of Programming Massively Parallel Processors [@pmpp], an excellent resource and probably the canonical text on this topic. 
...
__global__ void gemm_naive_kernel(const half* A, const half* B, half* C, // <1>
                                  int M, int N, int K, 
    ...
    if (row < M && col < N) {
        ...
        for (int i = 0; i < K; i++) {
            val += __half2float(A[row * K + i]) * __half2float(B[i * N + col]); // <2>
        }
...
2. **The Bottleneck**: This line is the performance killer. For every single pixel in C, we are fetching the entire row of A and column of B from Global Memory (DRAM). 
:::
...
![Memory hierarchy of an A100-40GB [@memoryhierarchy]](images/02_a100_memory.png){width=100%}
...
![Visualization of tiled matrix multiplication [@tiledmatmul]](images/03_tiled_mm.png){width=95%}
...
![Tensor Core performing a 4x4x4 matrix multiply and accumulate operation [@tensorcores]](images/04_tensor_core.png){width=90%}
...
3. **Accumulator Grid**: Accumulator is a 2 x 2 grid because each warp is assigned a 32 x 32 output tile but can only compute 16 x 16 at a time. 
...
* [x] Compute - We're using the Tensor Cores to perform matrix multiplication and accumulation. 
* [x] Registers - The Tensor Cores have their own dedicated registers to store data for the compute operation, so we're not slowed down by loading data from shared memory for the operation.
* [x] Shared memory - We're loading in large tiles from global memory at once per block and reusing as much data as possible between warps. 
* [x] Memory latency hiding - With our double buffer, we're ensuring we're computing as much of the time as possible while we wait on memory to load. 
...
__global__ void gemm_swizzled_kernel(const half* A, const half* B, half* C, int M, int N, int K, float alpha, float beta) {
    ...
    int panel_number = idx_linear / (swizzle_factor * grid_m_blocks); // <2>
    ...
    auto load_tile_async = [&](int stage_idx, int k_step) { // <5>
    ...
    for (int k = 0; k < K; k += BLOCK_K) {
        ...
        #pragma unroll
        for (int k_step = 0; k_step < BLOCK_K; k_step += WMMA) { // <6>
...
```
1. **Newspaper Panels**: So our "newspaper panels" have a width of 4 blocks. This is a standard balanced choice: if we have too narrow of a panel, we are effectively traversing column-major, and if we have too wide of a panel, we might as well just traverse row-major. 
2. **Grid Swizzling**: Computing the panel index beforehand tells us what column the left side of the current panel starts at. At the end of these few lines, we have our new block row and column index in terms of our output matrix, that traverses our newspaper columns first instead of going row-major.
3. **Shared Memory Swizzling**: This is where we swizzle the shared memory, by adding padding to our shared memory declaration.
4. **Doubled Fragments**: We double the number of fragments so we can load all the data at once the warp is using for its math loop into separate fragments, and then do all the math. 
5. **Async Loading Lambda**: We moved the async loading to a lambda for readability. 
6. **Doubled Data Loading**: Since we're loading twice the data now per K-loop iteration, we need a new k-step loop that performs the warp math twice.
7. **Warp Math**: We load the data all at once before this into our 4 fragments, and then can just loop 4 times calling ```mma_sync``` to perform the math. 
8. **Dimension Check**: Since we doubled BLOCK_K we need to change this dimension check too. It may seem disappointing that we are now handling even fewer matrices with our optimized kernel. Don't worry, we'll fix this in the next kernel!
:::
...
__global__ void gemm_swizzled_all(const half* A, const half* B, half* C, int M, int N, int K, float alpha, float beta) {
    ...
    auto load_tile_async = [&](int stage_idx, int k_step) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            bool a_fully_valid = (global_row_a < M) && (global_col_a + 7 < K); // <1>
...
```
1. **Valid flag**: We check if we are far enough within bounds to still do a vectorized load, or if we would go beyond the edges of the input matrices. If we're far enough within bounds, we can issue our ````pipeline_memcpy_async```` command as before. 
2. **Zero padding**: If we're too close to the edge of the matrix, we loop element by element, loading from global memory where we're still in bounds and padding with zeroes wherever we're not. 
3. **Boundary check for writing**: We have the same boundary checks as usual to not write out zeroes or junk to global memory in the epilogue.
:::
...
```{python}
#| label: fig-gpu-optimization
#| fig-cap: "GPU Runtime by Kernel Optimization Step"
...
# 1. Setup Data
data = [
    ["NVIDIA T4", "Naive", 0.5, 8.49], ["NVIDIA A100 (80GB)", "Naive", 0.5, 1.03], ["NVIDIA H100 (SXM)", "Naive", 0.5, 0.54], ["NVIDIA H200 (SXM)", "Naive", 0.5, 0.53], ["NVIDIA B200", "Naive", 0.5, 0.50],
    ["NVIDIA T4", "Tiled", 8, 6.73], ["NVIDIA A100 (80GB)", "Tiled", 8, 0.72], ["NVIDIA H100 (SXM)", "Tiled", 8, 0.37], ["NVIDIA H200 (SXM)", "Tiled", 8, 0.36], ["NVIDIA B200", "Tiled", 8, 0.33],
    ["NVIDIA T4", "WMMA", 8, 1.68], ["NVIDIA A100 (80GB)", "WMMA", 8, 0.17], ["NVIDIA H100 (SXM)", "WMMA", 8, 0.10], ["NVIDIA H200 (SXM)", "WMMA", 8, 0.10], ["NVIDIA B200", "WMMA", 8, 0.10],
    ["NVIDIA T4", "Double Buffer", 32, 1.04], ["NVIDIA A100 (80GB)", "Double Buffer", 32, 0.12], ["NVIDIA H100 (SXM)", "Double Buffer", 32, 0.05], ["NVIDIA H200 (SXM)", "Double Buffer", 32, 0.05], ["NVIDIA B200", "Double Buffer", 32, 0.05],
    ["NVIDIA T4", "Swizzled", 32, 0.49], ["NVIDIA A100 (80GB)", "Swizzled", 32, 0.07], ["NVIDIA H100 (SXM)", "Swizzled", 32, 0.04], ["NVIDIA H200 (SXM)", "Swizzled", 32, 0.04], ["NVIDIA B200", "Swizzled", 32, 0.03],
]
df = pd.DataFrame(data, columns=["GPU Model", "Kernel", "Arithmetic Intensity (FLOP/B)", "Runtime (ms)"])

# 2. Aesthetics
plt.figure(figsize=(7, 6))
sns.set_theme(style="whitegrid")

gpu_list = df["GPU Model"].unique()
colors = sns.color_palette("tab10", n_colors=len(gpu_list))
gpu_colors = dict(zip(gpu_list, colors))

# Define the explicit order for the X-axis
kernel_order = ["Naive", "Tiled", "WMMA", "Double Buffer", "Swizzled"]

# Create Label Strings with Intensity
x_labels = []
for k in kernel_order:
    # Find the intensity for this kernel
    intensity = df[df["Kernel"] == k]["Arithmetic Intensity (FLOP/B)"].iloc[0]
    x_labels.append(f"{k}\n({intensity} FLOP/B)")

# 3. Plotting
for gpu in gpu_list:
    subset = df[df["GPU Model"] == gpu].copy()
    
    # Map 'Kernel' string to an integer index (0, 1, 2, 3, 4)
    subset["x_pos"] = subset["Kernel"].apply(lambda x: kernel_order.index(x))
    subset = subset.sort_values("x_pos")
    
    # Plot Line
    plt.plot(subset["x_pos"], subset["Runtime (ms)"], 
             color=gpu_colors[gpu], label=gpu, linewidth=2, alpha=0.8, zorder=1)
    
    # Plot Markers (Uniform circles now)
    plt.scatter(subset["x_pos"], subset["Runtime (ms)"], 
                color=gpu_colors[gpu], marker='o', 
                s=150, zorder=2, edgecolor='white', linewidth=0.5)

# 4. Legend (Just for GPU Model now)
gpu_legend_elements = [Line2D([0], [0], color=gpu_colors[gpu], lw=2, label=gpu) for gpu in gpu_list]
plt.legend(handles=gpu_legend_elements, title="GPU Model", loc="upper right")

# 5. Axis & Ticks
plt.yscale('log')
plt.ylabel("Runtime (ms) - Log Scale", fontsize=11)
plt.title("GPU Runtime Optimization Path", fontsize=13)

# Set X-ticks to the integer positions
plt.xticks(range(len(kernel_order)), x_labels, fontsize=10)
plt.xlabel("Kernel Optimization Stage", fontsize=11)

# Custom Y-ticks for readability
ax = plt.gca()
ax.yaxis.set_major_formatter(ticker.ScalarFormatter())
ax.set_yticks([0.03, 0.05, 0.1, 0.5, 1, 5, 10])
...
!["ShaderShinobi" is my pseudonym. I debated whether to omit the other leaderboard usernames for anonymity, but those usernames already look pretty pseudonymous. Also, I'm hoping if either of those authors see this post, they'll contact me to nerd out about GPUs.](images/05_leaderboard.png){width=90%}


## Further Optimizations
I can almost certainly ascertain that the author of the next best solution had a generally superior kernel though, as their solutions are public. In particular, they used Warp Group MMA, a capability introduced in the Hopper generation that is much more efficient than standard WMMA. I'm quite certain that their solution would be generally faster on a large test set. The cleanest way to use Warp Group MMA is with an external library, which is prohibited by the problem constraints so I considered it out of scope for this problem. Admirably, this author went ahead and called it directly with PTX code. While I assumed this would be very messy, their solution was surprisingly still quite nice to read. The architecture of Hopper and Blackwell GPUs is quite different and more optimized than previous generations for GEMM operations. In a future post, I will explore Warp Group MMA, the CuTe library, and various optimizations only available on the current generation of GPUs.
