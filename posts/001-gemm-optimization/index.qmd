...
__global__ void gemm_buffer_kernel(const half* A, const half* B, half* C, int M, int N, int K, float alpha, float beta) {
    ...
    // We treat the A and B tiles, 64 x 16 and 16 x 64 respectively, as linear arrays of 128 8-element vectors.
    ...
    // Threads move data from Shared (float) to Global (half)
    // We have 64 * 64 = 4096 total elements to process and 128 threads. 
    // We must process 32 elements per thread.
    // If we vectorize this into processing 8 elements per step, we need only 4 steps per thread.
